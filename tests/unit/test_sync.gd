...
#     フィールドが増えれば全ハッシュがずれる
# 10. 壁反射とネット衝突で ball_attack_kind / ball_health_damage をクリア。
...
# 2026-08-02 Claude Codeレビューで、サーブのセルフトスだけが古いhit_kindを
# 再利用する経路を発見。TOSSへ必ず更新し、ユーザー承認後に実測更新。
#   3071785023893461452 -> 5042647433660423328
# 2026-08-02 #114 Task 1で戦闘リソースの同期欄と単調IDを追加。挙動はまだ
# 新状態を読まない。更新前583本中、直列化長と状態ハッシュを固定する2本、
# 第2状態遷移ゴールデンの計3本だけが赤で、残り580本と新規往復検査は緑。
# SCRIPT ERRORは0件。状態列追加だけを原因として実測更新。
#   5042647433660423328 -> 499200927738241715
# 2026-08-02 #114 Task 2でドライブを0..100へ移行し、旧自然回復欄を削除。
# 機能検査586本が緑で、状態形式を固定する2本だけが赤の状態から実測更新。
#   499200927738241715 -> -360030157049021182
# 2026-08-02 #114 Task 3で有効通常攻撃15と攻撃後回復窓を接続。
# 新規境界10本を含む機能検査596本が緑で、状態ハッシュ2本だけが赤の
# 状態から実測更新。SCRIPT ERRORは0件。
#   -360030157049021182 -> 8756143912408634954
# 2026-08-02 #114 Task 5で保持IDと防御返球10を接続。
# 新規境界6本を含む機能検査603本が緑で、状態ハッシュ2本だけが赤の
# 状態から実測更新。SCRIPT ERRORは0件。
#   8756143912408634954 -> -6476463211007448277
# 2026-08-02 #114 Task 6で予約5、先読み・反応、10tick構えを接続。
# 新規境界10本を含む機能検査614本が緑で、同期ゴールデンだけが赤の
# 状態から実測更新。SCRIPT ERRORは0件。
#   -6476463211007448277 -> -7085514558860277894
# 2026-08-02 #114 Task 7で飛びつき15・弱体版、ブロック5+5・体力版、
# ソフトブロック識別状態を接続。機能検査622本が緑で、同期ゴールデンと
# 第2状態遷移ゴールデンだけが赤の状態から実測更新。SCRIPT ERRORは0件。
#   -7085514558860277894 -> -3349805604857385944
# 2026-08-02 #114 Task 8で体力100、300tickスタンへ移行し、旧連打状態を
# 同期列から削除。新規境界7本を含む機能検査627本が緑で、同期ゴールデンと
# 第2状態遷移ゴールデンだけが赤の状態から実測更新。SCRIPT ERRORは0件。
#   -3349805604857385944 -> -3607501786886565490
# 2026-08-02 #114 Task 9で同期診断5値とCPU飛びつき選択を接続。
# 新規UI・CPU・1ラリー統合検査を含む機能検査636本が緑で、直列化長、
# 第2状態遷移ゴールデン、同期ゴールデンだけが赤から実測更新。
#   -3607501786886565490 -> -2498959315247485757
# 2026-08-02 原作必殺技Task 2で同期状態の形を拡張した実測値。
# 3600tickの物理入力列は不変で、個別機能648本と状態検査6本の緑を確認して更新。
# 2026-08-03: CPUプロファイルbit 56..58へ地上必殺方針を追加。
# 機能検査707本が緑で、状態ハッシュ2本だけが赤の状態から実測更新。
# 2026-08-03: UMEを基準に全キャラ共通重力と高度別の自然な滞空時間へ変更。
# 決定論検査と全ジャンプ・CPU機能検査が緑であることを確認して実測更新。
# 2026-08-03: 固有ジャンプ高度を廃止し、A=200、B=170、C=140、D=120、
# E=110の共通表へ移行。高さ200pxまでの敵陣返球保証に合わせ、返球上限の
# 高さ係数を1/2から13/25へ再実測調整した。機能検査718本と決定論検査が緑、
# SCRIPT ERROR 0件で、同期ゴールデンだけが赤の状態から実測更新。
const GOLDEN_COMBINED_HASH := -660413482496460521
