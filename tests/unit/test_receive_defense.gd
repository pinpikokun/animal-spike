...
func _resolve_contact(attack_kind: int, contact: int, base_damage: int,
		burnout: bool = false, health: int = 100, dive_input: int = 0,
		d2: int = 0) -> Array:
	var w := _world()
	var s = w[0]
	var cfg = w[1]
	...
	s.ball_health_damage = base_damage
...
func test_normal_attack_carries_power_health_damage() -> void:
	for row in [[Chars.CHAR_SEC2, 35], [Chars.CHAR_PANDA, 25], [Chars.CHAR_PIYO, 15]]:
		...
		check_eq(s.ball_attack_kind, SimState.BALL_ATTACK_NORMAL,
			"通常アタック属性: %s" % [row])
		check_eq(s.ball_health_damage, row[1],
			"POWER基礎体力損害: %s" % [row])
...
func test_power_rank_health_damage_matrix() -> void:
	...
	for i in bases.size():
		...
		check_eq(100 - toss.health, bases[i], "地上トス全量 rank=%d" % i)
		check_eq(100 - receive.health, receive_damage[i], "通常レシーブ40%% rank=%d" % i)
		check_eq(100 - just.health, 0, "ジャスト0 rank=%d" % i)
		check_eq(100 - dive.health, bases[i], "飛びつき全量 rank=%d" % i)

func test_burnout_has_no_extra_health_damage_matrix() -> void:
	var bases := [35, 30, 25, 20, 15]
	var receive_damage := [14, 12, 10, 8, 6]
	for i in bases.size():
...
func test_ground_toss_health_zero_stuns_without_blowback() -> void:
	var p = _resolve_contact(SimState.BALL_ATTACK_NORMAL, CONTACT_TOSS, 25,
		false, 25)[2]
	check(p.stun_ticks > 0, "地上トスのブレイクは気絶")
	check_eq(p.health, 0, "地上トスの体力スタン中は体力0")
	check_eq(p.vx, 0, "地上トスのブレイクは後方初速なし")
	check_eq(p.push, 0, "地上トスのブレイクは壁方向押し出しなし")

func test_normal_receive_health_zero_stuns_without_blowback() -> void:
	var p = _resolve_contact(SimState.BALL_ATTACK_JUST, CONTACT_RECEIVE, 25,
		false, 10)[2]
	check(p.stun_ticks > 0, "通常レシーブのブレイクは気絶")
	check_eq(p.health, 0, "通常レシーブの体力スタン中は体力0")
	check_eq(p.vx, 0, "通常レシーブのブレイクは後方初速なし")
	check_eq(p.push, 0, "通常レシーブのブレイクは壁方向押し出しなし")

func test_dive_health_zero_stuns_without_blowback() -> void:
	var result := _resolve_contact(SimState.BALL_ATTACK_JUST, CONTACT_DIVE, 25,
		false, 25)
	var cfg = result[1]
	var p = result[2]
	check(p.stun_ticks > 0, "飛びつきのブレイクは気絶")
	check_eq(p.health, 0, "飛びつきの体力スタン中は体力0")
...
func test_regular_attack_health_payload_does_not_damage_air_contacts() -> void:
	for input in [Simulation.IN_ACTION,
		...
		check_eq(p.health, 100, "空中接触は今回の体力損害対象外: input=%d" % input)

func test_regular_attack_health_payload_does_not_damage_blocker() -> void:
	var w := _world()
	var s = w[0]
	var cfg = w[1]
	var p = s.players[2]
	...
	check_eq(p.health, 100, "通常ブロックは体力損害対象外")
