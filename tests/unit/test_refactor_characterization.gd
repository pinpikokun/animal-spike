...
func test_intent_classification_table() -> void:
	# 操作原作回帰後の論理ベースライン:
	# 地上は下だけレシーブ、それ以外は横3トス。空中は上下=攻撃、なし=トス。
	var w: Array = _world()
	var cfg = w[1]
	...
	var rows := [
		...
		[1, Simulation.IN_RIGHT, edge_d2, 2, 0],
		[1, Simulation.IN_UP | Simulation.IN_RIGHT, near_d2, 2, 0],
		[0, Simulation.IN_DOWN, near_d2, 4, 0],
		[0, Simulation.IN_RIGHT, near_d2, 2, 0],
		[0, 0, near_d2, 1, 0],
	]
	for row in rows:
...
func test_output_velocity_snapshot() -> void:
	...
	check_eq(cases, [
		...
		[2, 0, 208622, -567978, 0, 100, 0],
		[4, 0, 1018429, 100489, 1, 100, 0],
		[4, 0, 900027, 149640, 0, 100, 0],
		[1, 0, 90830, -513365, 0, 100, 0],
...
func test_hit_chain_physics_state_transition_golden() -> void:
	...
	check_eq(_chain_hashes(), [
		...
		# 2026-08-02 成立したトス・返球・アタックをhit_kindへ記録した。
		# 物理値は不変だが、表示用状態を含むため全7値を承認後に実測更新。
		# 2026-08-02 #114 Task 1で未使用の同期欄と単調IDを追加したため、物理を
		# 変えず全7値が変化。更新前583本中この検査、直列化長、同期ゴールデンの
		# 3本だけが赤で、残り580本と新規往復検査は緑。SCRIPT ERRORは0件。
		# 2026-08-02 #114 Task 2でドライブを0..100へ移行し、旧回復欄を削除。
		# フィクスチャにも満タン100を明示し、既存のジャスト攻撃連鎖を維持した。
		# 機能検査586本が緑で、この検査と同期ゴールデンだけが赤の状態から実測更新。
		# 2026-08-02 #114 Task 3で攻撃ID、有効通常攻撃15、攻撃後回復窓を接続。
		# 新規境界10本を含む機能検査596本が緑で、状態ハッシュ2本だけが赤の
		# 状態から全7値を実測更新。SCRIPT ERRORは0件。
		...
		# 同期ゴールデンだけが赤の状態から全7値を実測更新。SCRIPT ERRORは0件。
		# 2026-08-02 #114 Task 9で開発診断5値を同期状態へ追加し、構え結果、
		# 体力損害、防御倍率、スタン終了理由を記録。機能検査636本が緑で、
		# この検査、直列化長、同期ゴールデンだけが赤から全7値を実測更新。
		# 2026-08-02 原作必殺技Task 2で選手5欄と特殊球6欄を追加し、旧球2欄を
		# 置換した。機能検査648本と新規状態検査6本が緑で、直列化レイアウトを
		# 固定するこの検査と同期ゴールデンだけが赤の状態から全7値を実測更新。
		# 2026-08-03 地上必殺方針をCPUプロファイルのbit 56..58へ追加した。
		# 既定4選手のcpu整数だけが変わり、機能検査707本が緑、この検査と
		# 同期ゴールデンだけが赤、SCRIPT ERROR 0件の状態から全7値を実測更新。
		# 2026-08-03 UMEの110px・上昇25tick・下降27tickを共通重力の基準にした。
		# 最初の5値は不変で、空中のマリオを16/23tick進めた最後の2値だけが変化。
		# ジャンプ弧以外の機能検査は緑の状態から実測更新。
		766543176468567452,
		-914998629844572214,
		-7871192741327269923,
		4143857915449755650,
