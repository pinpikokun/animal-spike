...
func test_hip_attack_hover_then_drop() -> void:
	...
	for t in 3:
		Sim.tick(s, [0, 0], cfg)
	check_eq(s.players[0].on_ground, 0, "空中にいる")
	# 空中+下(スペース無し)でヒップアタック発動→空中静止
	Sim.tick(s, [SimInput.IN_DOWN, 0], cfg)
	check(s.players[0].hip > 0, "ヒップアタックで空中静止 hip=%d" % s.players[0].hip)
	check_eq(s.players[0].vy, 0, "静止中は落下しない")
	# 静止が終わると急降下して着地
	var landed := false
	for t in 120:
		Sim.tick(s, [SimInput.IN_DOWN, 0], cfg)
	...
	check_eq(s.players[0].hip, 0, "着地でhip解除")
...
func test_hip_needs_hat() -> void:
	...
	for t in 3:
		Sim.tick(s, [0, 0], cfg)
	Sim.tick(s, [SimInput.IN_DOWN, 0], cfg)
	check_eq(s.players[0].hip, 0, "帽子が無ければヒップアタック不可")
...
func test_wall_cling_slides() -> void:
	...
	check(s.players[0].cling == 1, "左壁に張り付く cling=%d" % s.players[0].cling)
	check(s.players[0].vy > 0, "ずるずる降下 vy=%d" % s.players[0].vy)
