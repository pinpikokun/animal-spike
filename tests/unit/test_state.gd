...
func test_hash_changes_on_diff() -> void:
	...
	var d = SimState.new()
	d.players[1].drive_reserved = 1
	check(a.state_hash() != d.state_hash(), "予約ドライブ差分でも変わる")
	var e = SimState.new()
...
func test_load_int_array_roundtrip() -> void:
	...
	a.players[2].x = 999
	a.players[2].burn = 37
	a.players[2].drive_gauge = 65
	a.players[2].drive_reserved = 5
	...
	check_eq(b.players[2].drive_gauge, 65, "ドライブゲージ残量の復元")
	check_eq(b.players[2].drive_reserved, 5, "予約ドライブ量の復元")
