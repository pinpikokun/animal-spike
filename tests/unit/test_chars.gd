...
func test_super_catalog_has_complete_non_resource_fields() -> void:
	...
	check_eq(Chars.super_def(Chars.SUPER_FLAME_ATTACK).defense_class,
	...
	check_eq(Chars.super_def(Chars.SUPER_GHOST_BALL).condition,
		...
		"ゴーストボールは地上上+D")
	check_eq(Chars.super_def(Chars.SUPER_GHOST_BALL).defense_class,
		Chars.DEFENSE_DELAYED, "ゴーストボールは時間差系")
