...
func test_drive_gauge_rules_loaded() -> void:
	...
	check_eq(cfg.special_damage_default, 22, "必殺技の標準体力ダメージは22")
	check_eq(cfg.shock_ticks, 60, "感電は60tick")
	check_eq(cfg.bubble_ticks, 60, "泡は60tick")
	check_eq(cfg.original_tick_rate_milli, 28200, "原作tickレートは28.2Hz")
