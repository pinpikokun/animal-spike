...
func state_hash() -> int:
	...
	for v in to_int_array():
		# ロールバックの熱い経路なので、固定8バイトを展開して内側ループを省く。
		# 演算順は従来と同一で、ゴールデンハッシュ試験が互換性を固定する。
		h ^= v & 0xFF; h *= 1099511628211
		h ^= (v >> 8) & 0xFF; h *= 1099511628211
		h ^= (v >> 16) & 0xFF; h *= 1099511628211
		h ^= (v >> 24) & 0xFF; h *= 1099511628211
		h ^= (v >> 32) & 0xFF; h *= 1099511628211
		h ^= (v >> 40) & 0xFF; h *= 1099511628211
		h ^= (v >> 48) & 0xFF; h *= 1099511628211
		h ^= (v >> 56) & 0xFF; h *= 1099511628211
	return h
