...
### 回答1への自己指摘(追加検証で発見): serve_strike分岐との順序問題

`_apply_hit(s, best_i, ...)` の呼び出し直後、`_resolve_hit` 内には
serve_strike分岐が残っており `s.phase = PHASE_RALLY` を設定する。
今日の実行順は「(タッチ超過なら)授与=POINT_PAUSE設定 → serve_strike=RALLY設定」
であり、授与をresolver復帰後へ遅らせるとこの順序が逆転する。
同一tickで両者が併発し得るなら最終phaseが変わる=挙動変更になる。

5aの実施要件に次を追加する:

1. サーブ打撃とタッチ超過の同時発生が到達可能かをコードで解析する。
2. 到達可能なら、授与呼び出しは `_apply_hit` 直後(serve_strike分岐の前)へ
   置く設計とし、`_resolve_hit` は「得点チーム」を戻り値でなくローカルで
   処理する形を再設計する(Codexと再協議)。
3. 到達不能と証明できても、その根拠(タッチ数の遷移経路)を特性テストとして
   固定し、将来のルール変更で暗黙に壊れないようにする。

ゴールデンはこの経路を踏まない可能性が高く、ハッシュ不変だけでは
この順序逆転を検出できない。だからこそ事前解析を5aの合格条件に含める。
