...
Claude Codeが現物照合済み)
状態: 第4訂=Codex最終依存監査(codex-final-dependency-audit.md)のDASH定数4参照を
反映済み。他の変更なし=Codexの最終判定条件を満たす。ユーザーの開始承認待ち。
承認前は工程1を含め一切実施しない
...
| 4 | player_movement.gd抽出 | `_step_player`+ジャンプ補助関数+移動系定数(RUN/BRAKE/DASH/HIP/CLING等)+`PUSH_UNIT_PX`/`PUSH_DECAY` を逐語移動。テスト直接参照を同コミットで更新: `test_char_stats.gd` の `Sim._jump_height_px` 3参照(32行に2つ+99行)、`test_dash.gd` のDASH定数4参照(33/37/59/64行: `DASH_SPD_PCT`x2/`DASH_TAP_WINDOW`/`DASH_TICKS`)を `PlayerMovement` へ(preload追加)。simulationに互換定数は残さない | 全テスト緑(test_dash/test_skid/test_hip_cling/test_hatを明記)、ゴールデン不変 |
