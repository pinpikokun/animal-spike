...
4. resetなしでは2回目でダッシュし、resetを挟むと同方向の最初の1タップで
   ダッシュしない対照付き意図テストを追加する。
5. 承認済みの実測記録と新値で合成ゴールデンを1件だけ更新する。
6. `run_tests.ps1` を再実行し、他の固定値を含む全件が通ることを確認する。
7. 状態漏れ修正をコミットする。
...
7. `run_tests.ps1` を再実行し、全件と固定値が通り、#91の新ゴールデン値が
   RESET側だけの測定値から動かないことを確認する。
8. 設定検査修正をコミットする。
